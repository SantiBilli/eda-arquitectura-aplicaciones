...
- **No llegan correos de SNS**  
  - Revisá que la suscripción está **Confirmed**.  
  - En *SES sandbox*, los emails deben estar **verificados** (remitente/destino).

- **EventBridge no dispara**  
  - Mirá *Metrics* de la regla, y *Matched events*.  
  - Confirmá `Source` y `DetailType` exactos.

- **DynamoDB falla con `ConditionalCheckFailedException`**  
  - Probablemente el `status` no está en el esperado (p.ej. aprobar dos veces).  
  - Devolvemos **409** con HTML explicativo.

- **`ValidationException` por expresiones**  
  - Si un atributo se llama `items`, en `UpdateExpression` usá alias (`ExpressionAttributeNames`, `#items`).
...
├── lambdas/
│   ├── compras_crear_orden/
│   ├── casacentral_procesar_orden/
│   ├── notificaciones_oc/
│   ├── aprobar_orden/
│   ├── rechazar_orden/
│   ├── notif_proveedor/
│   ├── notif_deposito/
│   ├── deposito_aceptar_recepcion/
│   └── logistica_confirmar_despacho/
├── docs/
│   └── arquitectura.md
└── README.md
```

> Si más adelante querés, lo pasamos a **IaC** (SAM/CloudFormation/Terraform/Serverless) y agregamos un pipeline.

---

## 📄 Licencia

Este proyecto se publica con fines educativos/demostrativos. Ajustá permisos y manejo de datos sensibles antes de usarlo en producción.
